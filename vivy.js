const vivy = (() => {
      ...
      if (elem.scope) {
        ...
        if (data == null) {
          if (tree.children.size > 0)
            console.warn(
              `Hydration failure: null found at $.${scope.join(".")}`,
              element
            );
  ...
  function adjustArrayTree(subtree, data, path, hydrate) {
    ...
    const subtreeSize = [...children.keys()].filter((key) =>
      /\d+/.test(key)
    ).length;
    ...
    if (subtreeSize > arr.length) {
      ...
      toUpdated.push(
        ...toRemoveProps.map((prop) => [prop, children.get(prop)])
      );
  ...
  function updateDom(subtree, data, path, hydrate) {
    ...
    let node,
      nodes = [[subtree, data, path]];
    while ((node = nodes.pop())) {
      const [subtree, data, path] = node;

      const isArray = !!subtree.templates;
      if (isArray) {
        const toUpdate = adjustArrayTree(subtree, data, path, hydrate).map(
  ...
  function parseAttributesToPaths(elem) {
    ...
    for (const attr of attrs) {
      ...
      if (
        ...
        if (scopePath)
          console.warn(`Multiple scope paths found on element: ${attr}`, elem);
        ...
        showIfPath = {
          negated,
          path: toParts(name.slice(start, end)),
          attr: name,
        };
    ...
    if (attrs[":show-if"]?.value) {
      ...
      showIfPath = {
        negated,
        path: toParts(value.slice(start, end)),
        attr: ":show-if",
      };
    }
  ...
  function createSyncer(root, path, elem) {
    ...
    return (_) => {
      const siblings = node.elements.filter(
        (elem) => elem.element.type == "checkbox"
      );
  ...
  function createArraySubtrees(elem, subtree, array, path, suffix) {
    if (!Array.isArray(array))
      console.warn(`Non-array found at $.${path.join(".")}:`, array, elem);
  ...
  function findTemplates(elemRoot) {
    ...
    while ((elem = elems.pop())) {
      if (elem.tagName == "VIVY:TEMPLATE") {
        ...
        if (rtn.has(name))
          throw new Error(`Duplicate template name found: ${name}`);
  ...
  function throwOnExcessiveTemplateUsage(name) {
    if (templateUsageCount.size == 0)
      setTimeout(() => templateUsageCount.clear(), 0);

    const count = templateUsageCount.get(name) ?? 0;

    if (count > 10_000) {
      throw new Error(
        `Template "${name}" inserted >10_000 times (possible infinite loop)`
      );
    }
  ...
  function createProxyTree(elem, rootData) {
    ...
    const createProxy = (path, initValue) => {
      ...
      const proxy = new Proxy(target, {
        get(_, prop) {
          ...
          if (value == null)
            console.warn(`Read property of ${value} (reading '${prop}')`);
        ...
        set(_, prop, value) {
          ...
          if (parent == null)
            throw new Error(
              `Cannot set properties of ${value} (reading '${prop}')`
            );
          ...
          if (node.templates && parent.length !== prevLength) {
            ...
            parent.length !== prevLength &&
            node.children.has("length")
          ) {
    ...
    const createEventHandler = (elem, handlerPath, contextPath) => {
      return (event) => {
        ...
        if (handler instanceof Function) {
          ...
          console.warn(
            `Non-function event-handler: "${handler}" found at $.${path}`,
            elem
          );
        }
      };
    ...
    const traverseToPath = (root, tree, data, path, traversal) => {
      ...
      for (const part of traversal) {
        ...
        if (subData && !(part in subData)) {
          const nearMiss = Object.keys(subData).find(
    ...
    const hydrate = (elements, data, path, startRoot) => {
      ...
      let node,
        nodes = elements.map((elem) => [elem, hydrateRoot, data, path]);
      while ((node = nodes.shift())) {
        ...
        if (
          elem.tagName.startsWith("T:") ||
          elem.tagName.startsWith("TEMPLATE:")
        ) {
          ...
          const insertedElems = insertTemplate(
            elem,
            template,
            scopePath,
            showIfPath
          ...
          nodes.unshift(
            ...insertedElems.map((elem) => [elem, tree, data, path])
          );

          continue;
        }

        const scopeTraversal =
          scopePath && traverseToPath(root, tree, data, path, scopePath);
        ...
        if (assignToPath) {
          ...
          if (untraversed.length !== 0)
            throw new Error(`Can't assign-to an array, ${elem}`);
        ...
        if (showIfPath) {
          ...
          if (untraversed.length !== 0)
            throw new Error(`Can't show-if an array, ${elem}`);
        ...
        for (const { event, path } of eventPaths) {
          ...
          if (untraversed.length !== 0)
            throw new Error(
              `Event handler path can't reference arrays, ${elem}`
            );
          if (!(subData instanceof Function))
            console.warn(
              `Non-function found for event handler: ${subData}`,
              elem
            );
        ...
        if (showIfPath) {
          ...
          if (untraversed[0] == "[]") {
            ...
            const trees = createArraySubtrees(
              elem,
              subtree,
              subData,
              traversed,
            ...
            const children = Array.from(elem.children);
            nodes.push(
              ...children.map((child) => [child, subtree, subData, traversed])
            );
